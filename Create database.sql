...
INSERT INTO `client` VALUES (1,4,'mantenimiento.png','1234',1),(2,3,'mantenimiento.png','1234',1),(3,6,'tecnico.png','1234',2),(4,5,'tecnico.png','1234',2),(5,2,'tecnico.png','1234',2);
...
CREATE TABLE `maintenancerequest` (
  ...
  KEY `maintenancerequest_FK_1` (`idWorker`),
  KEY `maintenancerequest_FK_2` (`idPlace`),
  CONSTRAINT `maintenancerequest_FK` FOREIGN KEY (`idMachine`) REFERENCES `machine` (`idMachine`),
  CONSTRAINT `maintenancerequest_FK_1` FOREIGN KEY (`idWorker`) REFERENCES `worker` (`idWorker`),
  CONSTRAINT `maintenancerequest_FK_2` FOREIGN KEY (`idPlace`) REFERENCES `place` (`idPlace`)
...
CREATE TABLE `sessionclient` (
  ...
  `datesession` timestamp NOT NULL DEFAULT current_timestamp(),
...
--
-- Temporary view structure for view `view_calendar_data`
--

DROP TABLE IF EXISTS `view_calendar_data`;
/*!50001 DROP VIEW IF EXISTS `view_calendar_data`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `view_calendar_data` AS SELECT 
 1 AS `idMaintenanceOrder`,
 1 AS `stepValue`,
 1 AS `date`,
 1 AS `start`,
 1 AS `finish`,
...
--
-- Temporary view structure for view `view_maintenanceprocedure_data`
--

DROP TABLE IF EXISTS `view_maintenanceprocedure_data`;
/*!50001 DROP VIEW IF EXISTS `view_maintenanceprocedure_data`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `view_maintenanceprocedure_data` AS SELECT 
 1 AS `idMaintenanceProcedure`,
 1 AS `title`,
 1 AS `workload`,
 1 AS `roadmaps`,
 1 AS `law`,
...
--
-- Temporary view structure for view `view_maintenancereport_data`
--

DROP TABLE IF EXISTS `view_maintenancereport_data`;
/*!50001 DROP VIEW IF EXISTS `view_maintenancereport_data`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `view_maintenancereport_data` AS SELECT 
 1 AS `idMaintenanceOrder`,
 1 AS `startReport`,
 1 AS `finishReport`,
 1 AS `timeCount`,
...
--
-- Temporary view structure for view `view_maintenancerequest_data`
--

DROP TABLE IF EXISTS `view_maintenancerequest_data`;
/*!50001 DROP VIEW IF EXISTS `view_maintenancerequest_data`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `view_maintenancerequest_data` AS SELECT 
 1 AS `idMaintenanceRequest`,
 1 AS `title`,
 1 AS `description`,
 1 AS `image`,
 1 AS `requestDate`,
...
INSERT INTO `worker` VALUES (2,'00000000','OPERARIO','INDUSTRIAL',25,1,4,2),(3,'11223344','JEFE','MANTENIMIENTO',2750,1,1,1),(4,'12345678','ADMINISTRADOR','GENERAL',0,1,1,1),(5,'55555555','DEPP QUISPE ','JHONNY',42.5,1,3,2),(6,'11111111','CARRY MESAS','JIM',55,1,2,2);
...
--
-- Final view structure for view `view_calendar_data`
--
...
/*!50001 VIEW `view_calendar_data` AS select `m`.`idMaintenanceOrder` AS `idMaintenanceOrder`,`m`.`stepValue` AS `stepValue`,cast(convert_tz(`m`.`startOrder`,'+00:00','-05:00') as date) AS `date`,cast(convert_tz(`m`.`startOrder`,'+00:00','-05:00') as time) AS `start`,cast(convert_tz(`m`.`finishOrder`,'+00:00','-05:00') as time) AS `finish`,`vm`.`title` AS `title`,`vm`.`longname` AS `longname`,`p`.`name` AS `name`,`p`.`color` AS `color` from ((`maintenanceorder` `m` join `view_maintenanceprocedure_data` `vm` on(`m`.`idMaintenanceProcedure` = `vm`.`idMaintenanceProcedure`)) join `priority` `p` on(`m`.`idPriority` = `p`.`idPriority`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `view_maintenanceprocedure_data`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_maintenanceprocedure_data` AS select `m`.`idMaintenanceProcedure` AS `idMaintenanceProcedure`,`m`.`title` AS `title`,`m`.`workload` AS `workload`,`m`.`roadmaps` AS `roadmaps`,`m`.`law` AS `law`,`m2`.`idMachine` AS `idMachine`,`m2`.`name` AS `nameMachine`,`p`.`idPart` AS `idPart`,`p`.`name` AS `namePart`,`s`.`idSubPart` AS `idSubPart`,`s`.`name` AS `nameSubPart`,`st`.`name` AS `nameStrategy`,`pl`.`name` AS `namePlace`,`w`.`idWorker` AS `idWorker`,concat(`w`.`lastname`,', ',`w`.`firstname`) AS `longname` from ((((((`maintenanceprocedure` `m` join `subpart` `s` on(`m`.`idSubPart` = `s`.`idSubPart`)) join `part` `p` on(`s`.`idPart` = `p`.`idPart`)) join `machine` `m2` on(`p`.`idMachine` = `m2`.`idMachine`)) join `strategy` `st` on(`m`.`idStrategy` = `st`.`idStrategy`)) join `place` `pl` on(`m`.`idPlace` = `pl`.`idPlace`)) join `worker` `w` on(`m`.`idWorker` = `w`.`idWorker`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `view_maintenancereport_data`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_maintenancereport_data` AS select `r`.`idMaintenanceOrder` AS `idMaintenanceOrder`,`r`.`startReport` AS `startReport`,`r`.`finishReport` AS `finishReport`,`r`.`timeCount` AS `timeCount`,`r`.`observation` AS `observation`,`r`.`image` AS `image`,`m`.`startOrder` AS `startOrder`,`m`.`finishOrder` AS `finishOrder`,`vmd`.`title` AS `title`,`vmd`.`workload` AS `workload`,`vmd`.`nameMachine` AS `nameMachine`,`vmd`.`namePart` AS `namePart`,`vmd`.`nameSubPart` AS `nameSubpart`,`vmd`.`nameStrategy` AS `nameStrategy`,`vmd`.`namePlace` AS `namePlace`,`vmd`.`longname` AS `longname` from ((`maintenancereport` `r` join `maintenanceorder` `m` on(`r`.`idMaintenanceOrder` = `m`.`idMaintenanceOrder`)) join `view_maintenanceprocedure_data` `vmd` on(`m`.`idMaintenanceProcedure` = `vmd`.`idMaintenanceProcedure`)) where `m`.`idMaintenanceOrder` <> 0 */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `view_maintenancerequest_data`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_maintenancerequest_data` AS select `m`.`idMaintenanceRequest` AS `idMaintenanceRequest`,`m`.`title` AS `title`,`m`.`description` AS `description`,`m`.`image` AS `image`,`m`.`requestDate` AS `requestDate`,`m`.`requestState` AS `requestState`,`w`.`idWorker` AS `idWorker`,concat(`w`.`lastname`,', ',`w`.`firstname`) AS `longname`,`p`.`name` AS `placeName`,`m`.`idMachine` AS `idMachine`,`m2`.`name` AS `machineName` from (((`maintenancerequest` `m` join `place` `p` on(`m`.`idPlace` = `p`.`idPlace`)) join `worker` `w` on(`m`.`idWorker` = `w`.`idWorker`)) join `machine` `m2` on(`m`.`idMachine` = `m2`.`idMachine`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_searchworkerclient` AS select `w`.`idWorker` AS `id`,`c`.`idClient` AS `idClient`,`w`.`dni` AS `dni`,concat(`w`.`lastname`,', ',`w`.`firstname`) AS `largename`,`p`.`name` AS `name`,`c`.`image` AS `image`,`c`.`pass` AS `pass`,`c`.`levelWork` AS `levelWork` from (((`worker` `w` join `client` `c` on(`w`.`idWorker` = `c`.`idWorker`)) join `worktype` `w2` on(`w`.`idWorkType` = `w2`.`idWorkType`)) join `profesion` `p` on(`w`.`idProfesion` = `p`.`idProfesion`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2023-07-11  1:48:43
